...
export function Topbar({
  ...
  return (
    <header className="sticky top-0 z-20 hidden h-[68px] items-center justify-between gap-4 border-b border-navy-100 bg-white px-6 lg:flex">
      <h1 className="truncate font-mono text-[12px] font-medium uppercase tracking-eyebrow text-navy-900">{match?.label ?? 'Panel'}</h1>

      <div className="flex items-center gap-3">
        <button
          type="button"
          className="relative grid h-10 w-10 place-items-center rounded-lg text-navy-300 transition-colors hover:bg-mist hover:text-navy-700"
        ...
        >
          ...
          {unread > 0 && (
            <span
              className={cn(
                'absolute right-1.5 top-1.5 grid min-w-[16px] place-items-center bg-gold-400 px-1 font-mono text-[9px] font-medium tabular-nums text-navy-900',
              )}
            >
              {unread > 9 ? '9+' : unread}
            </span>
